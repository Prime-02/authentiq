        ...
        {Products.map((product, ind) => {
          ...
          return (
            <div key={ind}>
              ...
              <div className="flex gap-x-5 min-w-full overflow-x-auto scrollbar-none pb-16">
                {value.img.map((img, imgInd) => (
                  <Link
                  href={`/product/${img.title.toLowerCase().replace(/\s+/g, "-")}`}
                  passHref
                    key={imgInd}
                    className="flex-shrink-0 flex flex-col min-h-full items-center justify-between p-5 bg-white shadow-2xl overflow-hidden  rounded-lg"
                  >
                    {/* Image */}
